...
#import "SPZShuffleIntention.h"
#import "SPZConstants.h"
#import "SPZMultiGameHomeViewController.h"
...
@interface SPZGameBoardViewController ()<UIImagePickerControllerDelegate, UINavigationControllerDelegate>

@property (nonatomic, strong) SPZPuzzleBoard *puzzleBoardView;
...
@property (nonatomic, strong) IBOutlet SPZShuffleIntention *shuffleIntent;
...
@implementation SPZGameBoardViewController
...
- (void)initCommon
{
    ...
    self.model.tilesMatrix = [[SPZTilesMatrix alloc] initWithNumColumns:NumColumns withNumRows:NumRows];
...
- (void)displaySliceImagesFor:(UIImage*)srcImage
{
    ...
    SPZPuzzleBoard *boardView = [[SPZPuzzleBoard alloc] initWithFrame:containerFrame];
    ...
    for (int i=0; i<NumRows; i++) {
        for (int j=0; j<NumColumns; j++) {
            SPZTile *tileImageView = [self createTile:sliceImageList[index] tileX:j tileY:i];
...
- (SPZTile*)createTile:(UIImage*)sliceImage tileX:(NSInteger)xPos tileY:(NSInteger)yPos
{
    ...
    SPZTile *tile = [[SPZTile alloc] initWithFrame:frame];
...
- (void)checkPuzzlesMatching
{
    ...
    for (SPZTile *tile in [self.model.tilesMatrix flattenTilesArray] ){
        if (![tile isMatched]) {
            return;
        }
    }
...
- (void)addGestureRecogizer:(SPZTile*)tile
{
    // tap
...
{
    SPZTile *senderTile = (SPZTile*) gestureRecognizer.view;
...
{
    SPZTile *senderTile = (SPZTile*) dragGestureRecognizer.view;
